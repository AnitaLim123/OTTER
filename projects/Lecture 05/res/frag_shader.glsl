...
void main() {
    ...
    float spec = pow(max(dot(N, halfwayDir), 0.0), 128); // Shininess coefficient (can be a uniform)    vec3 specular = specularStrength * spec * lightColor; // Can also use a specular color
